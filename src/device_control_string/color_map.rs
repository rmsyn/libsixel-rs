...
pub const fn u8_percentage(val: u8) -> u8 {
    let ret = ((val as u16) * 100 + 127) / 255;
    if ret > 100 {
        100
    } else {
        ret as u8
...
#[repr(C)]
#[derive(Clone, Copy, Debug, Default, PartialEq)]
pub struct BasicColor(usize);

impl From<usize> for BasicColor {
...
impl From<&BasicColor> for usize {
...
impl From<&[u8]> for ColorMap {
    fn from(val: &[u8]) -> Self {
        ...
        val.chunks_exact(3)
            .enumerate()
            .take(num)
            .map(|(i, rgb)| ColorMapItem::create_rgb(i, rgb[0], rgb[1], rgb[2]))
