...
pub fn dither_func_atkinson(data: &mut [u8], width: usize) -> Result<()> {
    ...
    if max_offset > data_len {
        ...
        let (error_r, error_g, error_b) = (
            (data[0] & 0x7).saturating_add(4),
            (data[1] & 0x7).saturating_add(4),
            (data[2] & 0x7).saturating_add(4),
        );
...
pub fn apply_15bpp_dither(
    ...
    match method_for_diffuse {
        MethodForDiffuse::Fs if x < width.saturating_sub(1) && y < height.saturating_sub(1) => {
            dither_func_fs(pixels, width)
        }
        MethodForDiffuse::Atkinson
            if x < width.saturating_sub(2) && y < height.saturating_sub(2) =>
        {
            dither_func_atkinson(pixels, width)
        }
        MethodForDiffuse::Jajuni if x < width.saturating_sub(2) && y < height.saturating_sub(2) => {
            dither_func_jajuni(pixels, width)
        }
        MethodForDiffuse::Stucki if x < width.saturating_sub(2) && y < height.saturating_sub(2) => {
            dither_func_stucki(pixels, width)
        }
        MethodForDiffuse::Burkes if x < width.saturating_sub(2) && y < height.saturating_sub(2) => {
